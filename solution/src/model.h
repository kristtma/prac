...
public:
    ...
    void MarkActive(std::chrono::milliseconds game_time) noexcept {
        last_active_game_time_ = game_time;
    }

    std::chrono::milliseconds GetLastActiveTime() const noexcept {
        return last_active_game_time_;
    }
...
private:
    ...
    int score_ = 0;
    bool retired_ = false;
    std::chrono::milliseconds last_active_game_time_{0};
...
public:
    ...
    void Tick(int time_delta_ms) {
        current_game_time_ += std::chrono::milliseconds(time_delta_ms);
        
        std::cout << "GameSession::Tick: dt=" << time_delta_ms 
                << "ms, new current_time=" << current_game_time_.count() << "ms" << std::endl;
        
        for (auto& dog : dogs_) {
            std::cout << "  Dog '" << dog.GetName() << "': speed=(" 
                    << dog.GetSpeedX() << ", " << dog.GetSpeedY() << ")" << std::endl;
            
            // Обновляем время активности только если собака движется
            if (dog.GetSpeedX() != 0.0 || dog.GetSpeedY() != 0.0) {
                std::cout << "    Marking active (moving)" << std::endl;
                dog.MarkActive(current_game_time_);
            }
            
            dog.Move(time_delta_ms / 1000.0, map_.GetRoads());
        }
    }
    ...
    std::chrono::milliseconds GetCurrentGameTime() const noexcept { return current_game_time_; }
    void AddGameTime(std::chrono::milliseconds dt) noexcept { current_game_time_ += dt; }
private:
    ...
    std::chrono::milliseconds current_game_time_{0};
...
public:
    ...
    std::chrono::seconds GetDogRetirementTime() const noexcept { return dog_retirement_time_; }
    void SetDogRetirementTime(std::chrono::seconds t) noexcept { dog_retirement_time_ = t; }
    using DogRetiredCallback = std::function<void(const Dog&, const Map&)>;
    void SetDogRetiredCallback(DogRetiredCallback cb) { dog_retired_cb_ = std::move(cb); }
...
private:
    ...
    DogRetiredCallback dog_retired_cb_;
    std::chrono::seconds dog_retirement_time_{60}; // по умолчанию 60 секунд
