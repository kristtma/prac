...
http_server::StringResponse RequestHandler::HandleGameRecords(const http_server::StringRequest& req) {
    unsigned start = 0;
    unsigned max_items = 100;

    auto target_str = std::string(req.target());
    if (auto pos = target_str.find('?'); pos != std::string::npos) {
        auto query = target_str.substr(pos + 1);

        // Парсим start
        if (auto s_pos = query.find("start="); s_pos != std::string::npos) {
            auto end = query.find('&', s_pos);
            auto val = query.substr(s_pos + 6, end == std::string::npos ? std::string::npos : end - (s_pos + 6));
            try {
                start = static_cast<unsigned>(std::stoul(val));
            } catch (...) {
                return MakeBadRequestResponse("Invalid 'start' parameter");
            }
        }

        // Парсим maxItems
        if (auto m_pos = query.find("maxItems="); m_pos != std::string::npos) {
            auto end = query.find('&', m_pos);
            auto val = query.substr(m_pos + 9, end == std::string::npos ? std::string::npos : end - (m_pos + 9));
            try {
                max_items = static_cast<unsigned>(std::stoul(val));
            } catch (...) {
                return MakeBadRequestResponse("Invalid 'maxItems' parameter");
            }
        }
    }

    if (max_items > 100) {
        return MakeBadRequestResponse("maxItems must not exceed 100");
    }

    try {
        auto records = db_->LoadRecords(start, max_items);
        json::array arr;
        for (const auto& r : records) {
            arr.push_back(json::object{
                {"name", r.name},
                {"score", r.score},
                {"playTime", r.play_time_seconds}
            });
        }

        http_server::StringResponse res;
        ...
        return MakeJsonResponse(http::status::internal_server_error, "internalError", "Database error");
    }
}
...
http_server::StringResponse RequestHandler::HandleGameTick(const http_server::StringRequest& req){
    ...
    try{
        ...
        // ТОЛЬКО ЭТОТ ВЫЗОВ - удалить цикл session.Tick() выше
            ...
            if (move_str == "L") dir = Direction::WEST;
                ...
                if (session) {
                    ...
                    if (dog) {
                        ...
                        // ВАЖНО: обновляем время активности при команде остановки
                        dog->MarkActive(session->GetCurrentGameTime());
                        std::cout << "Dog '" << dog->GetName() << "' command '" << move_str 
                        << "', marking active at game_time: " 
                        << session->GetCurrentGameTime().count() << "ms" << std::endl;
                    }
                }
            } else {
                return MakeBadRequestResponse("Failed to parse action");
            ...
            if (move_str != "") {
                ...
                if (session) {
                    ...
                    if (dog) {
                        ...
                        // ВАЖНО: обновляем время активности при команде движения
                        dog->MarkActive(session->GetCurrentGameTime());
                        std::cout << "Dog '" << dog->GetName() << "' command '" << move_str 
                        << "', marking active at game_time: " 
                        << session->GetCurrentGameTime().count() << "ms" << std::endl;
...
http_server::StringResponse RequestHandler::HandleJoinGame(const http_server::StringRequest& req) {
    ...
    try {
        ...
        // Создаем собаку
        model::Dog::Id dog_id{std::to_string(game_.GetSessions().size() + game_.GetMaps().size())};
        model::Dog dog{dog_id, user_name};
        dog.SetPlayerId(player_id);
        
        // ВАЖНО: устанавливаем начальное время активности
        dog.MarkActive(session->GetCurrentGameTime());  // Используем текущее время сессии
        std::cout << "New dog '" << dog.GetName() << "' joined at game_time: " 
                        << session->GetCurrentGameTime().count() << "ms" 
                        << ", last_active set to: " << dog.GetLastActiveTime().count() << "ms" 
                        << std::endl;
        ...
        auto current_game_time = session->GetCurrentGameTime();

        // Сохраняем с временем присоединения
        token_to_player_.emplace(token, PlayerInfo{
            user_name, 
            map_id, 
            player_id,
            current_game_time  // Сохраняем игровое время присоединения
        });

        // Также сохраняем в player_join_times_ для обратной совместимости
        player_join_times_[player_id] = current_game_time;
        
        size_t bag_capacity = map->GetBagCapacity();
        added_dog.SetBagCapacity(bag_capacity);
